import PDFDocument from "pdfkit";

import { csvRows } from "@/lib/export/csv";
import { getOptionalServerEnv } from "@/lib/env";
import { createAdminClient } from "@/lib/supabase/server";

export type ExportType = "csv_transactions" | "gst_summary" | "pdf_summary";

type ExportRecord = {
  id: string;
  firm_id: string;
  client_id: string | null;
  gst_period_id: string | null;
  export_type: ExportType;
  metadata: Record<string, unknown> | null;
  created_at: string;
};

type TransactionRow = {
  id: string;
  transaction_type: string;
  status: string;
  transaction_date: string | null;
  party_name: string | null;
  party_gstin: string | null;
  invoice_number: string | null;
  category: string | null;
  taxable_amount: number | null;
  cgst_amount: number | null;
  sgst_amount: number | null;
  igst_amount: number | null;
  cess_amount: number | null;
  total_amount: number | null;
  approved_at: string | null;
};

type GstPeriodRecord = {
  id: string;
  firm_id: string;
  client_id: string;
  period_start: string;
  period_end: string;
  filing_type: string;
  status: string;
  clients:
    | {
        business_name: string | null;
        gstin: string | null;
      }
    | Array<{
        business_name: string | null;
        gstin: string | null;
      }>
    | null;
  gst_summaries:
    | {
        sales_taxable_amount: number | null;
        purchase_taxable_amount: number | null;
        output_cgst: number | null;
        output_sgst: number | null;
        output_igst: number | null;
        input_cgst: number | null;
        input_sgst: number | null;
        input_igst: number | null;
        net_tax_payable: number | null;
        mismatch_count: number | null;
        missing_document_count: number | null;
        generated_at: string | null;
      }
    | Array<{
        sales_taxable_amount: number | null;
        purchase_taxable_amount: number | null;
        output_cgst: number | null;
        output_sgst: number | null;
        output_igst: number | null;
        input_cgst: number | null;
        input_sgst: number | null;
        input_igst: number | null;
        net_tax_payable: number | null;
        mismatch_count: number | null;
        missing_document_count: number | null;
        generated_at: string | null;
      }>
    | null;
};

function requiredString(value: unknown, field: string) {
  if (typeof value !== "string" || !value.trim()) {
    throw new Error(`Export metadata is missing ${field}.`);
  }

  return value.trim();
}

function amount(value: number | null | undefined) {
  return Number(value ?? 0);
}

function currency(value: number | null | undefined) {
  return new Intl.NumberFormat("en-IN", {
    style: "currency",
    currency: "INR",
    maximumFractionDigits: 2,
  }).format(amount(value));
}

function single<T>(value: T | T[] | null | undefined) {
  return Array.isArray(value) ? value[0] : value;
}

function configuredPositiveInteger(key: string, fallback: number) {
  const value = Number(getOptionalServerEnv(key));
  return Number.isFinite(value) && value > 0 ? Math.floor(value) : fallback;
}

function exportRowLimit() {
  return configuredPositiveInteger("EXPORT_MAX_TRANSACTION_ROWS", 10_000);
}

function exportByteLimit() {
  return configuredPositiveInteger("EXPORT_MAX_FILE_BYTES", 10 * 1024 * 1024);
}

async function pdfBuffer(build: (doc: PDFKit.PDFDocument) => void) {
  const doc = new PDFDocument({ margin: 48, size: "A4" });
  const chunks: Buffer[] = [];

  doc.on("data", (chunk: Buffer | Uint8Array) =>
    chunks.push(Buffer.from(chunk)),
  );

  const finished = new Promise<Buffer>((resolve, reject) => {
    doc.on("end", () => resolve(Buffer.concat(chunks)));
    doc.on("error", reject);
  });

  build(doc);
  doc.end();

  return finished;
}

async function generateTransactionsCsv({
  supabase,
  firmId,
  clientId,
  periodStart,
  periodEnd,
  snapshotAt,
}: {
  supabase: NonNullable<ReturnType<typeof createAdminClient>>;
  firmId: string;
  clientId: string;
  periodStart: string;
  periodEnd: string;
  snapshotAt: string;
}) {
  const maxRows = exportRowLimit();
  const baseQuery = supabase
    .from("transactions")
    .select("id", {
      count: "exact",
      head: true,
    })
    .eq("firm_id", firmId)
    .eq("client_id", clientId)
    .eq("status", "approved")
    .gte("transaction_date", periodStart)
    .lte("transaction_date", periodEnd)
    .lte("approved_at", snapshotAt);

  const { count, error: countError } = await baseQuery;

  if (countError) {
    throw new Error(countError.message);
  }

  if ((count ?? 0) > maxRows) {
    throw new Error(
      `Export contains ${count} approved transactions, above the configured limit of ${maxRows}. Narrow the period or increase EXPORT_MAX_TRANSACTION_ROWS after capacity verification.`,
    );
  }

  const { data, error } = await supabase
    .from("transactions")
    .select(
      "id, transaction_type, status, transaction_date, party_name, party_gstin, invoice_number, category, taxable_amount, cgst_amount, sgst_amount, igst_amount, cess_amount, total_amount, approved_at",
    )
    .eq("firm_id", firmId)
    .eq("client_id", clientId)
    .eq("status", "approved")
    .gte("transaction_date", periodStart)
    .lte("transaction_date", periodEnd)
    .lte("approved_at", snapshotAt)
    .order("transaction_date", { ascending: true });

  if (error) {
    throw new Error(error.message);
  }

  const transactions = (data ?? []) as TransactionRow[];

  return {
    body: Buffer.from(
      csvRows(
        [
          "Transaction ID",
          "Date",
          "Type",
          "Party",
          "GSTIN",
          "Invoice",
          "Category",
          "Taxable",
          "CGST",
          "SGST",
          "IGST",
          "Cess",
          "Total",
        ],
        transactions.map((transaction) => [
          transaction.id,
          transaction.transaction_date,
          transaction.transaction_type,
          transaction.party_name,
          transaction.party_gstin,
          transaction.invoice_number,
          transaction.category,
          amount(transaction.taxable_amount),
          amount(transaction.cgst_amount),
          amount(transaction.sgst_amount),
          amount(transaction.igst_amount),
          amount(transaction.cess_amount),
          amount(transaction.total_amount),
        ]),
      ),
      "utf8",
    ),
    fileName: `transactions-${periodStart}-to-${periodEnd}.csv`,
    contentType: "text/csv; charset=utf-8",
    clientId,
    metadata: {
      row_count: transactions.length,
      row_limit: maxRows,
      period_start: periodStart,
      period_end: periodEnd,
      snapshot_approved_at_lte: snapshotAt,
    },
  };
}

async function loadGstPeriod({
  supabase,
  firmId,
  periodId,
}: {
  supabase: NonNullable<ReturnType<typeof createAdminClient>>;
  firmId: string;
  periodId: string;
}) {
  const { data, error } = await supabase
    .from("gst_periods")
    .select("*, clients(business_name, gstin), gst_summaries(*)")
    .eq("id", periodId)
    .eq("firm_id", firmId)
    .single();

  if (error || !data) {
    throw new Error(error?.message ?? "GST period not found.");
  }

  return data as GstPeriodRecord;
}

function gstSummaryCsv(period: GstPeriodRecord) {
  const client = single(period.clients);
  const summary = single(period.gst_summaries);

  if (!summary) {
    throw new Error("Generate the GST summary before exporting it.");
  }

  return Buffer.from(
    csvRows(
      ["Field", "Value"],
      [
        ["Client", client?.business_name ?? "Unknown client"],
        ["GSTIN", client?.gstin ?? ""],
        ["Period start", period.period_start],
        ["Period end", period.period_end],
        ["Filing type", period.filing_type],
        ["Readiness", period.status],
        ["Sales taxable", amount(summary.sales_taxable_amount)],
        ["Purchase taxable", amount(summary.purchase_taxable_amount)],
        ["Output CGST", amount(summary.output_cgst)],
        ["Output SGST", amount(summary.output_sgst)],
        ["Output IGST", amount(summary.output_igst)],
        ["Input CGST", amount(summary.input_cgst)],
        ["Input SGST", amount(summary.input_sgst)],
        ["Input IGST", amount(summary.input_igst)],
        ["Net tax payable", amount(summary.net_tax_payable)],
        ["Mismatches", amount(summary.mismatch_count)],
        ["Missing documents", amount(summary.missing_document_count)],
        ["Generated at", summary.generated_at ?? ""],
        ["Direct GST filing submitted", "No"],
      ],
    ),
    "utf8",
  );
}

async function gstSummaryPdf(period: GstPeriodRecord) {
  const client = single(period.clients);
  const summary = single(period.gst_summaries);

  if (!summary) {
    throw new Error("Generate the GST summary before exporting it.");
  }

  return pdfBuffer((doc) => {
    doc.fontSize(18).text("KhataOne GST Summary", { continued: false });
    doc.moveDown(0.5);
    doc
      .fontSize(10)
      .fillColor("#5f6b61")
      .text(
        "Review and export preparation only. This document does not confirm GST portal filing or submission.",
      );
    doc.moveDown();
    doc.fillColor("#17201a").fontSize(12);
    doc.text(`Client: ${client?.business_name ?? "Unknown client"}`);
    doc.text(`GSTIN: ${client?.gstin ?? "Pending"}`);
    doc.text(`Period: ${period.period_start} to ${period.period_end}`);
    doc.text(`Filing type: ${period.filing_type}`);
    doc.text(`Readiness: ${period.status.replaceAll("_", " ")}`);
    doc.moveDown();

    const rows = [
      ["Sales taxable", currency(summary.sales_taxable_amount)],
      ["Purchase taxable", currency(summary.purchase_taxable_amount)],
      ["Output CGST", currency(summary.output_cgst)],
      ["Output SGST", currency(summary.output_sgst)],
      ["Output IGST", currency(summary.output_igst)],
      ["Input CGST", currency(summary.input_cgst)],
      ["Input SGST", currency(summary.input_sgst)],
      ["Input IGST", currency(summary.input_igst)],
      ["Net tax payable", currency(summary.net_tax_payable)],
      ["Mismatches", String(summary.mismatch_count ?? 0)],
      ["Missing documents", String(summary.missing_document_count ?? 0)],
    ];

    for (const [label, value] of rows) {
      doc.font("Helvetica-Bold").text(label, { continued: true });
      doc.font("Helvetica").text(`  ${value}`, { align: "right" });
      doc.moveDown(0.25);
    }

    doc.moveDown();
    doc
      .fontSize(9)
      .fillColor("#5f6b61")
      .text(`Generated at: ${new Date().toLocaleString("en-IN")}`);
  });
}

async function buildExportFile({
  supabase,
  exportRecord,
}: {
  supabase: NonNullable<ReturnType<typeof createAdminClient>>;
  exportRecord: ExportRecord;
}) {
  const metadata = exportRecord.metadata ?? {};

  if (exportRecord.export_type === "csv_transactions") {
    return generateTransactionsCsv({
      supabase,
      firmId: exportRecord.firm_id,
      clientId:
        exportRecord.client_id ?? requiredString(metadata.client_id, "client_id"),
      periodStart: requiredString(
        metadata.requested_period_start ?? metadata.period_start,
        "period_start",
      ),
      periodEnd: requiredString(
        metadata.requested_period_end ?? metadata.period_end,
        "period_end",
      ),
      snapshotAt: exportRecord.created_at,
    });
  }

  const period = await loadGstPeriod({
    supabase,
    firmId: exportRecord.firm_id,
    periodId:
      exportRecord.gst_period_id ??
      requiredString(metadata.gst_period_id, "gst_period_id"),
  });
  const isPdf = exportRecord.export_type === "pdf_summary";

  return {
    body: isPdf ? await gstSummaryPdf(period) : gstSummaryCsv(period),
    fileName: `gst-summary-${period.period_start}-to-${period.period_end}.${isPdf ? "pdf" : "csv"}`,
    contentType: isPdf ? "application/pdf" : "text/csv; charset=utf-8",
    clientId: period.client_id,
    metadata: {
      gst_period_id: period.id,
      period_start: period.period_start,
      period_end: period.period_end,
      readiness: period.status,
      snapshot_generated_at: new Date().toISOString(),
    },
  };
}

export async function processExportGeneration(exportId: string) {
  const supabase = createAdminClient();

  if (!supabase) {
    return {
      ok: false,
      message: "Supabase service role is not configured.",
    };
  }

  const { data: exportRecord, error } = await supabase
    .from("exports")
    .select("id, firm_id, client_id, gst_period_id, export_type, metadata, created_at")
    .eq("id", exportId)
    .single();

  if (error || !exportRecord) {
    return {
      ok: false,
      message: error?.message ?? "Export record not found.",
    };
  }

  try {
    await supabase
      .from("exports")
      .update({ status: "processing" })
      .eq("id", exportRecord.id)
      .eq("firm_id", exportRecord.firm_id);

    const generated = await buildExportFile({
      supabase,
      exportRecord: exportRecord as ExportRecord,
    });

    const maxBytes = exportByteLimit();

    if (generated.body.byteLength > maxBytes) {
      throw new Error(
        `Export file is ${generated.body.byteLength} bytes, above the configured limit of ${maxBytes}. Narrow the export or increase EXPORT_MAX_FILE_BYTES after capacity verification.`,
      );
    }

    const storagePath = `${exportRecord.firm_id}/${exportRecord.id}/${generated.fileName}`;
    const { error: uploadError } = await supabase.storage
      .from("exports")
      .upload(storagePath, generated.body, {
        contentType: generated.contentType,
        upsert: true,
      });

    if (uploadError) {
      throw new Error(uploadError.message);
    }

    const { data: updated, error: updateError } = await supabase
      .from("exports")
      .update({
        client_id: generated.clientId,
        status: "completed",
        storage_path: storagePath,
        completed_at: new Date().toISOString(),
        metadata: {
          ...(exportRecord.metadata ?? {}),
          ...generated.metadata,
          file_name: generated.fileName,
          content_type: generated.contentType,
          byte_size: generated.body.byteLength,
          byte_limit: maxBytes,
          generated_at: new Date().toISOString(),
          generation_boundary: "export_generation_worker",
        },
      })
      .eq("id", exportRecord.id)
      .eq("firm_id", exportRecord.firm_id)
      .select("*")
      .single();

    if (updateError || !updated) {
      throw new Error(updateError?.message ?? "Could not finalize export.");
    }

    await supabase.from("audit_logs").insert({
      firm_id: exportRecord.firm_id,
      client_id: generated.clientId,
      actor_user_id: updated.requested_by ?? null,
      action: "export.generated",
      entity_type: "export",
      entity_id: exportRecord.id,
      after_data: updated,
      metadata: {
        export_type: exportRecord.export_type,
        direct_gst_filing: false,
        generation_boundary: "export_generation_worker",
      },
    });

    return {
      ok: true,
      message: "Export generated and stored.",
      exportId: exportRecord.id,
      storagePath,
    };
  } catch (error) {
    const message =
      error instanceof Error ? error.message : "Export generation failed.";

    await supabase
      .from("exports")
      .update({
        status: "failed",
        metadata: {
          ...(exportRecord.metadata ?? {}),
          error: message,
        },
      })
      .eq("id", exportRecord.id)
      .eq("firm_id", exportRecord.firm_id);

    return {
      ok: false,
      message,
      exportId: exportRecord.id,
    };
  }
}
