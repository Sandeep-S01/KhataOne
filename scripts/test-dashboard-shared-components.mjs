...
assert.match(designSystem, /export const filterInlineGridClassName =/);
assert.match(designSystem, /export const filterInlineControlClassName = "h-10 text-\[13px\] md:h-9 md:text-\[13px\]"/);
assert.match(designSystem, /export const filterInlineButtonClassName = "h-10 min-w-24 px-4 text-sm md:h-9"/);
assert.match(designSystem, /export const filterInlineActionsClassName =/);
assert.match(designSystem, /export function FilterInlineField/);
assert.match(designSystem, /export function FilterDateRangeField/);
...
assert.match(reviewQueue, /<FilterBar action="\/dashboard\/review-queue" className="min-w-0 gap-0 overflow-hidden p-0">/);
assert.match(reviewQueue, /<FilterPresetLink[\s\S]*Needs Review/);
assert.match(reviewQueue, /<FilterPresetLink[\s\S]*Duplicate Risk/);
assert.match(reviewQueue, /<FilterPresetLink[\s\S]*AI Draft/);
assert.match(reviewQueue, /Today[\s\S]*This Month[\s\S]*Q2 FY27/);
...
assert.match(reviewQueue, /<FilterGrid className=\{filterInlineGridClassName\}>/);
assert.match(reviewQueue, /<FilterInlineField label="Client" htmlFor="review-client">/);
assert.match(reviewQueue, /All Clients/);
assert.match(reviewQueue, /All Documents/);
assert.match(reviewQueue, /All Risk/);
assert.match(reviewQueue, /<FilterDateRangeField[\s\S]*fromId="review-from"[\s\S]*toId="review-to"/);
assert.match(reviewQueue, /<FilterActions className=\{filterInlineActionsClassName\}>/);
assert.match(reviewQueue, /<Button type="submit" size="sm" className=\{filterInlineButtonClassName\}>/);
assert.match(reviewQueue, /<ActionLink[\s\S]*href="\/dashboard\/review-queue"[\s\S]*size="sm"[\s\S]*className=\{filterInlineButtonClassName\}/);
assert.match(reviewQueue, /<TableToolbar/);
assert.match(reviewQueue, /Open exports/);
assert.match(reviewQueue, /Compact rows/);
assert.match(reviewQueue, /Fixed columns/);
