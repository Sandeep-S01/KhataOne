...
Dashboard header layout update (2026-09-13): Refined the protected dashboard header as a shared shell-only change. The header is square-edged and attached to the top of the dashboard section, with right-aligned search access, activity alert icon, profile context from the authenticated user, and the existing sign-out action. Controls follow the existing rounded-md design-system rhythm rather than heavy square boxes. Backend, Supabase, server actions, page data logic, and sidebar sizing remain unchanged.
