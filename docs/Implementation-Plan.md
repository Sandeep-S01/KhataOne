...
Production UI hotfix (2026-09-13): Review Queue and Inbox now fall back to firm-scoped read-only table queries when `search_review_queue` or `search_whatsapp_inbox` are absent from the live Supabase schema cache. The preferred RPC path remains active after migration, but the live UI no longer exposes raw PostgREST schema-cache errors while production migration application catches up.

Dashboard sidebar layout update (2026-09-13): Implemented the approved reference-inspired sidebar as a shared shell/component change only. `DashboardSidebar`, `DashboardNav`, `DashboardUtilityNav`, `DashboardMobileMenu`, `BrandLogo`, and the protected dashboard layout now use a rectangular flush-left desktop panel, 280px expanded / 76px collapsed rail, firm-context card, grouped operational navigation, and bottom Settings/Help utilities while preserving routes, server actions, Supabase access and dashboard page logic.

Dashboard header layout update (2026-09-13): Refined the protected dashboard header as a shared shell-only change. The header is square-edged, attached to the top of the dashboard section, aligned to the compact 64px sidebar logo area, and the outer dashboard gutters plus rounded main-content shell were removed, with right-aligned search access, activity alert icon, profile context from the authenticated user, and the existing sign-out action. Controls now follow the reference layout only through KhataOne tokens: icon-first search and alert controls, compact profile pill, subtle divider, and the existing sign-out action. Backend, Supabase, server actions, page data logic, and sidebar sizing remain unchanged.

Dashboard topbar interaction update (2026-09-14): Added a focused client component for topbar search, activity, and profile behavior. Search opens an accessible dialog and routes queries into existing dashboard pages that already support q filters; activity opens a menu of existing operational destinations. The sign-out server action remains in the server layout. No Supabase schema, backend route, server action, or sidebar width behavior changed.

UI consistency implementation plan (2026-09-14): Added docs/UI-Consistency-Implementation-Plan.md to sequence typography, theme color, icon treatment, and shared-component remediation. The plan prioritizes shared primitives, shell migration, dashboard metadata/status cleanup, source contracts, and visual verification without backend, Supabase, schema, or accounting workflow changes.
