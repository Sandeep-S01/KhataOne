...
Dashboard sidebar layout update (2026-09-13): Implemented the approved reference-inspired sidebar as a shared shell/component change only. `DashboardSidebar`, `DashboardNav`, `DashboardUtilityNav`, `DashboardMobileMenu`, `BrandLogo`, and the protected dashboard layout now use a rounded floating desktop panel, 280px expanded / 76px collapsed rail, firm-context card, grouped operational navigation, and bottom Settings/Help utilities while preserving routes, server actions, Supabase access and dashboard page logic.
