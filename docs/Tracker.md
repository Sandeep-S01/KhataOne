...
- Apply `supabase/migrations/20260811123000_relax_legacy_transactions_date.sql` in Supabase if Operations "Run now" reports a not-null violation on legacy `transactions.date`.
- Apply `supabase/migrations/20260811124500_relax_legacy_transactions_amount.sql` in Supabase if Operations "Run now" reports a not-null violation on legacy `transactions.amount`.
