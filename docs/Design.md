...
- Settings offers a browser-local Light, Dark, or System appearance preference for the protected dashboard only. Light is the initial choice even when the device prefers dark; System follows the device only when explicitly selected. Public and auth pages keep the light brand palette. Dashboard Dark uses neutral slate surfaces and restrained emerald actions, with the same scoped tokens applied to search and activity overlays. The preference is presentation-only and does not change firm data or accounting behavior.
...
- Tables: sticky header, compact rows, clear selection state, right-aligned amounts. Dashboard record tables show at most 10 rows per page and use the shared pagination controls to reach older rows while preserving active filters and detail context.
