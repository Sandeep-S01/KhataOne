...
Review Queue is the pilot page for the richer reference-style worklist pattern: a top status/date preset row, a full-width search row, compact second-row filters, equal-height Apply/Clear controls, and a table toolbar that exposes only truthful current-state labels until real selection or column-preference behavior exists. Status pills may show live firm-scoped counts, but unimplemented bulk approval, direct CSV export from the queue, or configurable columns must not be presented as working actions.
